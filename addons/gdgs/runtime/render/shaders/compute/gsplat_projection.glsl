...
const uint SCRATCH_PROJECTION_STAGE_POST_BARRIER_NO_SCRATCH_IMMEDIATE_RETURN = 1u << 13;
const uint SCRATCH_PROJECTION_STAGE_PRE_INSTANCE_READ_PROBE = 1u << 14;
const uint SCRATCH_PROJECTION_STAGE_INSTANCE_DATA_TOUCH = 1u << 15;
const uint SCRATCH_PROJECTION_STAGE_PRE_BLOCK_ENTER_PROBE = 1u << 16;
...
const int PROJECTION_MODE_POST_BARRIER_NO_SCRATCH_IMMEDIATE_RETURN_ONLY = 3;
const int PROJECTION_MODE_POST_BARRIER_IMMEDIATE_RETURN_ONLY = 4;
const int PROJECTION_MODE_PRE_BLOCK_ENTER_PROBE_ONLY = 5;
const int PROJECTION_MODE_INSTANCE_DATA_BLOCK_ONLY = 6;
const int PROJECTION_MODE_PRE_INSTANCE_READ_PROBE_ONLY = 7;
const int PROJECTION_MODE_INSTANCE_DATA_TOUCH_ONLY = 8;
const int PROJECTION_MODE_INSTANCE_DATA_ONLY = 9;
const int PROJECTION_MODE_MODEL_MATRIX_ONLY = 10;
const int PROJECTION_MODE_SPLAT_PAYLOAD_ONLY = 11;
const int PROJECTION_MODE_DUMMY_OUTPUT_WRITE_ONLY = 12;
...
void main() {
	...
	barrier();
	if (debug_projection_mode == PROJECTION_MODE_POST_BARRIER_NO_SCRATCH_IMMEDIATE_RETURN_ONLY) {
		return;
	}
	...
	atomicOr(scratch_probe[SCRATCH_PROBE_PROJECTION_STAGE_BITS], SCRATCH_PROJECTION_STAGE_PRE_BLOCK_ENTER_PROBE);
	if (debug_projection_mode == PROJECTION_MODE_PRE_BLOCK_ENTER_PROBE_ONLY) {
		return;
	}
	...
	atomicOr(scratch_probe[SCRATCH_PROBE_PROJECTION_STAGE_BITS], SCRATCH_PROJECTION_STAGE_PRE_INSTANCE_READ_PROBE);
	if (debug_projection_mode == PROJECTION_MODE_PRE_INSTANCE_READ_PROBE_ONLY) {
		return;
	}
	atomicOr(scratch_probe[SCRATCH_PROBE_PROJECTION_STAGE_BITS], SCRATCH_PROJECTION_STAGE_INSTANCE_DATA_TOUCH);
	uvec2 instance_data = splat_instance_data[id];
	if (debug_projection_mode == PROJECTION_MODE_INSTANCE_DATA_TOUCH_ONLY) {
		if (instance_data.x == 0xFFFFFFFFu && instance_data.y == 0xFFFFFFFFu) {
